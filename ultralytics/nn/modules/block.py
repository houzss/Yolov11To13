...
class ABlockv13(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=1.2, area=1):
        ...
        self.attn = AAttnv13(dim, num_heads=num_heads, area=area)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = nn.Sequential(Conv(dim, mlp_hidden_dim, 1), Conv(mlp_hidden_dim, dim, 1, act=False))
...
class A2C2fv13(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, a2=True, area=1, residual=False, mlp_ratio=2.0, e=0.5, g=1, shortcut=True):
        ...
        self.m = nn.ModuleList(
            nn.Sequential(*(ABlockv13(c_, num_heads, mlp_ratio, area) for _ in range(2))) if a2 else C3k(c_, c_, 2, shortcut, g) for _ in range(n)
        )
